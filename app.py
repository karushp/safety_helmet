...
# Load the YOLO model
model = YOLO("best_v8_25.pt")  # Path to the pre-trained YOLOv5 model

# Set up the Streamlit app
st.set_page_config(page_title="Safety Helmet Detection", page_icon="👷", layout="wide")
st.title("👷 Safety Helmet Detection")
st.markdown("""
This application performs real-time Safety Helmet Detection using the YOLOv8 model. Adjust the settings in the sidebar to customize the detection.
""")
...
def video_frame_callback(frame):
    ...
    cv2.putText(img, f"Helmet Count: {helmet_count}", (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 0), 2)
    cv2.putText(img, f"No Helmet Count: {no_helmet_count}", (10, 70), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 255), 2)
...
st.sidebar.markdown("""
- **Model:** YOLOv8
- **Developers:** Nick, Karush, Sayaka, Iliyas
